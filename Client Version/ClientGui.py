...
class ClickableNotification(QLabel):
    """A clickable notification label with hover effects"""
    clicked = pyqtSignal(str)  # Signal with notification type
...
class NotificationManager:
    """Manages positioning and displaying of notifications"""
    ...
    def show_notification(self, notification_type, message):
        ...
        notif = ClickableNotification(self.parent)
        notif._notification_type = notification_type
        notif.setTextFormat(Qt.RichText)
        ...
        QTimer.singleShot(5000, lambda: self.fade_out(notif))
        
        self.notifications.append(notif)
        
    def position_notification(self, notification):
        """Position notification in bottom-right with upward stacking"""
        if not self.parent:
            return
            
        # Get screen geometry (works better than window geometry)
        screen_rect = [self.parent.width(), self.parent.height()]
        x = screen_rect[0] - notification.width() - 20  # Right margin
        y = screen_rect[1] - 150  # Start from bottom
        
        # Stack upwards for existing notifications
    ...
    def fade_out(self, notification):
        """Gradually fade out and remove notification"""
        if notification in self.notifications:
            ...
            for notif in reversed(self.notifications):
                y -= notif.height() + self.spacing
                x = screen_rect[0] - notif.width() - 20
                print(x,max(y, 30))
                notif.move(x, max(y, 30))
        
    def on_notification_clicked(self, notification_type):
        """Handle notification clicks"""
        print(f"Notification clicked: {notification_type}")
        # Add your custom click handling here
        # Example: self.parent.show_transaction_details(notification_type)
...
class ClientGUI:
    ...
    def __setup_window(s):
        ...
        s.notif_manager = NotificationManager(s._window)
        s.notif_manager.show_notification(
        "received",
        f"Received 0.05 BTC\nFrom: 1A1zP1..."
            )
        s.notif_manager.show_notification(
