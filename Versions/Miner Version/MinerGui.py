...
class MinerGUI:
    ...
    def __create_elements(s):
        ...
        # Input label
        input_label = QLabel("Address to recieve mined reward:", s._central_widget)
    ...
    def __miner_start(s):
        ...
        if not check_address(address):
            s._err_label.setText("Invalid address")
            return 
        ...
        try:
            ...
            return
        send("Miner address:>" + address, s._skt)

        result = receive_buffer(s._skt)

        if result[1]=="Valid":
            s.__connect_event(address)
